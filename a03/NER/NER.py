...
class NER(object):
    ...
    def mmap_read_word_vectors(fname):
        ...
        mmap_obj = mmap.mmap(file_obj.fileno(), length=0,
    ...
    def __init__(self, training_file, test_file, model_file, word_vectors_file,
        ...
        self.pos_cache, self.w2id, self.vec_cache = {
        }, {}, np.zeros((NER.MAX_CACHE_SIZE, self.D))
        ...
        if training_file:
            ...
            if training_set:
                ...
                if stochastic_gradient_descent:
                    ...
                    b.fit()
                print("Model training took {}s".format(
...
def main():
    ...
    required_named.add_argument(
        '-w', type=str, required=True, help='file with word vectors')
    ...
    ner = NER(arguments.d, arguments.t, arguments.m, arguments.w,
              arguments.s, arguments.mgd, arguments.lowercase_fallback)
    ner.mo.close()
    ner.fo.close()
