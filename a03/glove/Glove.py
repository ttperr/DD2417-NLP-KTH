...
class Glove:
    ...
    def get_context(self, i):
        ...
        context = []
        for j in range(max(0, i-self.left_window_size), i):
            context.append(self.get_word_id(self.tokens[j]))
        for j in range(i+1, min(len(self.tokens), i+self.right_window_size+1)):
            context.append(self.get_word_id(self.tokens[j]))
        return context
    ...
    def process_files(self, file_or_dir):
        ...
        if os.path.isdir(file_or_dir):
            ...
            print(file_or_dir)
            stream = open(file_or_dir, mode='r',
    ...
    def compute_gradient(self, i, j):
        ...
        wi_vector_grad = self.f(self.X[i][j]) * (self.w_vector[i].T.dot(
            self.w_tilde_vector[j]) - math.log(self.X[i][j])) * self.w_tilde_vector[j]
        wj_tilde_vector_grad = self.f(self.X[i][j]) * (self.w_vector[i].T.dot(
            self.w_tilde_vector[j]) - math.log(self.X[i][j])) * self.w_vector[i]
    ...
    def train(self):
        ...
        while (self.patience > 0):
            ...
            iterations += 1
            i = random.choice(list(self.X.keys()))
...
def main():
    ...
    parser.add_argument('--file', '-f', type=str,
                        default='../RandomIndexing/data', help='The files used in the training.')
    parser.add_argument('--output', '-o', type=str, default='vectors.txt',
                        help='The file where the vectors are stored.')
    parser.add_argument('--left_window_size', '-lws', type=int,
                        default='2', help='Left context window size')
    parser.add_argument('--right_window_size', '-rws', type=int,
                        default='2', help='Right context window size')
    parser.add_argument('--continue_training', '-c', action='store_true',
                        default=False, help='Continues training from where it was left off.')

    arguments = parser.parse_args()

    glove = Glove(arguments.continue_training,
                  arguments.left_window_size, arguments.right_window_size)
